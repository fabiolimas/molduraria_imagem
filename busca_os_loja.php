...
<?php
	...
	$listar_envios="SELECT p.id_pedido, p.id_loja, p.valor, l.nome,p.cliente, p.os, p.data_loja, s.status_nome FROM pedido as p join lojas as l on l.id_loja=p.id_loja join status_os as s on s.id_status=p.id_status  where l.nome like '%$clientes%' or p.cliente like '%$clientes%' or p.os like '%$clientes%'  order by p.data_loja desc";
